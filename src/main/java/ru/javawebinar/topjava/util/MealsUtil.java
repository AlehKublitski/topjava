...
public class MealsUtil {
    private static MealServiceImpl mealService = new MealServiceImpl();
    private static int caloriesPerDayExample = 2000;
    static List<Meal> meals = mealService.listMeal();
...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static List<MealTo> getListMealToFromMeal() {
        Collection<List<Meal>> list = mealService.listMeal().stream()
                .collect(Collectors.groupingBy(Meal::getDate)).values();

        return list.stream().flatMap(dayMeals -> {
            boolean excess = dayMeals.stream().mapToInt(Meal::getCalories).sum() > caloriesPerDayExample;
