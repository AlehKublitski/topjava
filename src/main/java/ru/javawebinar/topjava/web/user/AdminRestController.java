...
public class AdminRestController extends AbstractUserController {
    ...
    public void update(@Valid @RequestBody User user, BindingResult result, @PathVariable int id) {
        ...
        if(result.hasErrors()) {
            throw new IllegalRequestDataException("Вы ввели неверные данные для изменения REST пользователя");
        }
       // try {
            super.update(user, id);
      /*  } catch (Exception e) {
            throw new IllegalRequestDataException("Вы ввели уже существующий e-mail" + e.getLocalizedMessage());
        }*/
    }
